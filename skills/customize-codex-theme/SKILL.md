...
4. **Apply/switch:** re-run `doctor`. After separate authorization and a graceful user exit, launch through **Codex Themes**, verify loopback ownership and renderer anchors, inject, then run `verify` before changing `loadedTheme`. On failure, remove partial injection and use **Codex Original**. Switching changes the next-launch state only; do not promise hot switching.
